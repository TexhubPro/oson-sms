...
- **Авторизация по Bearer-токену** или подпись **SHA-256 `str_hash`** — что выдал оператор
...
Если оператор выдал только hash-секрет без Bearer-токена, оставьте `token` пустым —
SDK подпишет запрос через `str_hash` и не отправит заголовок `Authorization`:
...
Хотя бы одно из `token` или `hash_secret` должно быть задано.
